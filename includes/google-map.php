...
function my_types( $fields ) {
    $fields['my_field'] = wpcf_fields_my_field();
    return $fields;
}
...
//        'inherited_field_type' => 'textfield',
        ...
        'meta_box_js' => array(// Add JS when field is active on post edit page
            'wpcf-jquery-fields-my-field' => array(
                'inline' => 'wpcf_fields_my_field_meta_box_js_inline', // This calls function that renders JS
        ...
        'group_form_js' => array(// Add JS when field is active on post edit page
            'wpcf-jquery-fields-my-field' => array(
                'inline' => 'wpcf_fields_my_field_group_form_js_inline', // This calls function that renders JS
                'deps' => array('jquery'), // (optional) Same as WP's enqueue_script() param
                'in_footer' => true, // (optional) Same as WP's enqueue_script() param
            ),
            'wpcf-jquery-fields-my-field' => array(
 ...
 * @return string
 */
function wpcf_fields_my_field_insert_form() {
 ...
 */
function wpcf_fields_my_field_meta_box_form( $data ) {
    $form['name'] = array(
 ...
 */
function wpcf_fields_my_field_editor_callback() {
    wp_enqueue_style( 'wpcf-fields-my_field',
    ...
    if ( isset( $_POST['_wpnonce_wpcf_form'] ) && wp_verify_nonce( $_POST['_wpnonce_wpcf_form'],
                    'wpcf-form' ) ) {
        add_action( 'admin_head_wpcf_ajax', 'wpcf_fields_my_field_editor_submit' );
    }
    ...
    <form method="post" action="">
        ...
        <?php wp_nonce_field( 'wpcf-form', '_wpnonce_wpcf_form' ); ?>
 ...
 */
function wpcf_fields_my_field_editor_submit() {
 ...
 * @param array $data
 */
function wpcf_fields_my_field_view( $data ) {
    $data['width'] = !empty( $data['width'] ) ? $data['width'] : 400;
    $data['height'] = !empty( $data['height'] ) ? $data['height'] : 280;
    return '<iframe style="border:1px solid black;" width="' . $data['width'] . '" height="' . $data['height']
            . '" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="http://maps.google.com/maps?q=loc:'
            . $data['field_value']
            . '&amp;num=1&amp;vpsrc=0&amp;hl=en&amp;ie=UTF8&amp;t=p&amp;z=15&amp;ll='
            . $data['field_value']
            . '&amp;output=embed&amp;iwloc="></iframe>';
    }
